...
export function SiteFooter() {
  return (
    <footer className="border-t border-line-faint py-8">
      <Container>
        <div className="flex flex-col items-center gap-2 text-center font-mono text-micro-sm text-ink-2 sm:flex-row sm:justify-between sm:text-left">
          <span>© {new Date().getFullYear()} SAM EHLERS</span>
          <span>BUILT WITH NEXT.JS · TYPESCRIPT · TAILWIND</span>
          <span>LAST DEPLOY {buildDate}</span>
        </div>
        <p className="mt-4 text-center text-fine text-ink-2 sm:text-left">
