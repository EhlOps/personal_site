...
export function CompanyContext({ context, companyName }: CompanyContextProps) {
  ...
  return (
    <aside className="border-l-2 border-line-strong bg-sunken p-3 sm:p-4">
          ...
          <div key={fact.label} className="flex items-baseline gap-1.5">
            ...
            <dd className="font-mono text-micro-lg text-ink-2" title={fact.detail}>
              ...
              {fact.attribution === "team" && (
                <span
                  className="ml-1.5 font-mono text-micro-sm text-ink-3"
      ...
      {primary.some((f) => f.detail) && (
        <ul className="mt-2 space-y-1">
          {primary
            .filter((f) => f.detail)
            .map((f) => (
              <li key={f.label} className="text-fine text-ink-3">
                <span className="font-mono text-micro-lg text-ink-3">{f.label}:</span> {f.detail}
              </li>
            ))}
        </ul>
      )}
      ...
      {deep.length > 0 && (
        <details className="mt-2 group">
          ...
          <ul className="mt-2 space-y-1">
            {deep.map((f) => (
              <li key={f.label} className="text-fine text-ink-3">
                <span className="font-mono text-micro-lg text-ink-3">{f.label}:</span> {f.value}
                {f.detail && <> ({f.detail})</>}
              </li>
            ))}
          </ul>
      ...
      {context.markIds && context.markIds.length > 0 && (
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-line-faint pt-2">
          {context.markCaption && (
      ...
      <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1 border-t border-line-faint pt-2">
        {dedupeSources(context.facts).map((source) => (
          <li key={source.url}>
            <a
              ...
              className="font-mono text-micro-sm text-ink-3 underline decoration-line-strong underline-offset-2 transition-colors hover:text-ink-2"
            >
              {source.label} ↗
            </a>
          </li>
