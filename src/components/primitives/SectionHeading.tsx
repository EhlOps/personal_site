...
export function SectionHeading({ id, index, eyebrow, title, lede, readout }: SectionHeadingProps) {
  return (
    <Reveal className="mb-6 sm:mb-7">
      ...
      <h2 id={`${id}-title`} className="mt-2 text-h2 text-ink">
        {title}
      </h2>
      {lede && <p className="mt-2 max-w-[var(--container-measure)] text-lead text-ink-2">{lede}</p>}
      <div aria-hidden className="relative mt-4 h-px w-full bg-line-faint">
